        ...
        <setting-item class='vertical-list-item' data-direction='column'>
          ...
          <div class='flex' data-id='${key}'>
            <input class='input' value='${
              color.value
            }' style='font-family: ${font}' placeholder='${color.title}' />
      ...
      <setting-section data-title='功能'>
          ${config.config.map((item) => {
            return `<setting-panel class='wrap'>
              <setting-list data-direction='column' data-title='${
                item.title
              }' is-collapsible>
  ...
  inputElements.forEach((element) => {
    element.addEventListener(
      ...
      debounce(async (event: Event) => {
        const key = (element.parentElement as HTMLElement).getAttribute(
          'data-id'
        );
